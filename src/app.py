...
@app.route('/')
def home_page(): # Более "человечное" имя для функции главной страницы
...
if __name__ == '__main__':
    # app.run() запускает встроенный сервер разработки Flask.
    # debug=True: активирует режим отладки (показывает ошибки в браузере, автоперезагрузка).
    # ВАЖНО: Никогда не используйте debug=True в рабочей (production) среде!
    # host='0.0.0.0': делает сервер доступным по всем сетевым интерфейсам компьютера,
    # а не только по localhost (127.0.0.1). Удобно для тестирования с других устройств
    # в локальной сети.
